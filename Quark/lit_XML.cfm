...
<CFPARAM NAME="form.begin_year" DEFAULT="2011">
<CFPARAM NAME="form.begin_day" DEFAULT="1">
<CFPARAM NAME="form.end_month" DEFAULT="4">
<CFPARAM NAME="form.end_year" DEFAULT="2011">
<CFPARAM NAME="form.end_day" DEFAULT="30">
<CFPARAM NAME="form.disp" DEFAULT="4">

<CFSET begin_date = CreateDate('2011', form.begin_month, form.begin_day)>
<CFSET type="d">
...
ON Org_Table.Org_Num = Event_Table.Org_Num) ON Venue_Table.ID = Event_Table.venueID WHERE ((((Event_Table.Start_Date)<=({ fn NOW() }+#Recordset1__begin_add#))AND((Event_Table.End_Date)>=({ fn NOW() }+#Recordset1__begin_add#)))AND 
Event_Table.ID=#Recordset1__disp_num#) OR ((((Event_Table.Start_Date) Between 
({ fn NOW() }+#Recordset1__begin_add#) AND({ fn NOW() }+#Recordset1__end_add#)))AND Event_Table.ID=#Recordset1__disp_num#) 
and (Event_Table.allowPublish = 1  OR org_table.printException = 1 OR venue_table.venue_zip in (select zip from tbl_AllowedZips))
