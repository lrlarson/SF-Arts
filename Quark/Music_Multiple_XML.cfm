...
ON Venue_Table.ID = Event_Table.venueID WHERE (((((Event_Table.Start_Date)<=({ fn NOW() }+#Recordset1__begin_add#))AND((Event_Table.End_Date)>=({ fn NOW() }+#Recordset1__begin_add#)))AND 
Event_Table.ID=#Recordset1__disp_num#)AND Event_Table.Date_Difference <> 0) OR (((((Event_Table.Start_Date) Between 
({ fn NOW() }+#Recordset1__begin_add#) AND({ fn NOW() }+#Recordset1__end_add#)))AND Event_Table.ID=#Recordset1__disp_num#) AND Event_Table.Date_Difference <> 0) and (Event_Table.allowPublish = 1 OR org_table.printException = 1 OR venue_table.venue_zip in (select zip from tbl_AllowedZips))
