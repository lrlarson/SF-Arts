...
<CFPARAM NAME="form.disp" DEFAULT="7">
...
({ fn NOW() }+#Recordset1__begin_add#) AND({ fn NOW() }+#Recordset1__end_add#)))AND Event_Table.ID=#Recordset1__disp_num#) and (Event_Table.allowPublish = 1 OR org_table.printException = 1 OR venue_table.venue_zip in (select zip from tbl_AllowedZips))
