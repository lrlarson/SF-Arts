...
SELECT Event_Table.Event_Name, Venue_Table.Venue_Name, Org_Table.Org_Name, Org_Table.Org_Web,Event_Table.ID,Event_Table.ID2,Venue_Table.Venue_address, 
...
ON Org_Table.Org_Num = Event_Table.Org_Num) ON Venue_Table.ID = Event_Table.venueID 
WHERE ((((Event_Table.Start_Date)<=(dbo.getPacificDateTime()+#Recordset1__begin_add#))AND((Event_Table.End_Date)>=(dbo.getPacificDateTime()+#Recordset1__begin_add#)))AND 
...
Disp = #Recordset1__disp_num# </CFOUTPUT> <cfoutput query="Recordset1"> 
  <table width="75%" border="1">
    <tr> 
      <td> 
	  	<p class="Body_Head">#Recordset1.Org_Name#</p>
